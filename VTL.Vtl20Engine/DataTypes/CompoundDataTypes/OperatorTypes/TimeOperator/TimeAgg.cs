...
{
    internal class TimeAgg : TimeAggBase
    {
        public TimeAgg(Duration? periodIndTo, Duration? periodIndFrom, Operand op, bool last)
        {
        ...
        private DataType PerformCalculation(DataSetType dataSet)
        {
            ...
            var measures = dataSet.DataSetComponents.Where(c => c.Role == ComponentType.ComponentRole.Measure).ToArray();
            ...
            using (var datasetEnumerator = dataSet.GetDataPointEnumerator())
            {
                ...
                do
                {
                    ...
                    if(sameIdentifiers && lastTimeValue != null && lastTimeValue.Equals(timeValue))
                    {
                        for (int i = 0; i < measureIndexes.Length; i++)
                        {
                            if (measures[i].DataType == typeof(NumberType))
                            {
                                aggregatedDataPoint[measureIndexes[i]] =
                                    (NumberType)aggregatedDataPoint[measureIndexes[i]] + (NumberType)dataPoint[measureIndexes[i]];
                            }
                            if (measures[i].DataType == typeof(IntegerType))
                            {
                                aggregatedDataPoint[measureIndexes[i]] =
                                    (IntegerType)aggregatedDataPoint[measureIndexes[i]] + (IntegerType)dataPoint[measureIndexes[i]];
                            }
                            /*
                            if (measures[i].DataType == typeof(StringType))
                            {
                                aggregatedDataPoint[measureIndexes[i]] =
                                    (StringType)aggregatedDataPoint[measureIndexes[i]] + (StringType)dataPoint[measureIndexes[i]];
                            }
                            */
                            // oldvalue + newvalue
                        }
