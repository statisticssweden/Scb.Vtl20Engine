...
namespace VTL.Vtl20Engine.Test.ComparisonOperatorTests
{
    ...
    public class LessThanTest
    {
        ...
        [TestCategory("Unit")]
        [TestMethod]
        public void LessThan_Constant()
        {
            var sut = new VtlEngine(new DataContainerFactory());

            var DS_r = sut.Execute("DS_r <- DS_1 < 250;", new[] { _ds_1 })
            ...
            using (var dataPointEnumerator = result.DataPoints.GetEnumerator())
            {
                ...
                Assert.AreEqual((BooleanType)false, dataPointEnumerator.Current[2]);//800

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((StringType)"Gross Prod.", dataPointEnumerator.Current[0]);
                Assert.AreEqual((StringType)"2014", dataPointEnumerator.Current[1]);
                Assert.AreEqual((BooleanType)false, dataPointEnumerator.Current[2]);//1000

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((StringType)"Population", dataPointEnumerator.Current[0]);
                Assert.AreEqual((StringType)"2013", dataPointEnumerator.Current[1]);
                Assert.AreEqual((BooleanType)true, dataPointEnumerator.Current[2]);//200

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((StringType)"Population", dataPointEnumerator.Current[0]);
                Assert.AreEqual((StringType)"2014", dataPointEnumerator.Current[1]);
                Assert.AreEqual((BooleanType)false, dataPointEnumerator.Current[2]);//250
            }
        }

        [TestCategory("Unit")]
        [TestMethod]
        public void LessThan_DataSet()
        {
            var sut = new VtlEngine(new DataContainerFactory());
            ...
            {
                dataPointEnumerator.MoveNext();
                Assert.AreEqual((BooleanType)false, dataPointEnumerator.Current[2]);//800<600

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((BooleanType)false, dataPointEnumerator.Current[2]);//1000<1000

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((BooleanType)false, dataPointEnumerator.Current[2]);//200<200

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((BooleanType)true, dataPointEnumerator.Current[2]);//250<300
            }
        }

        [TestCategory("Unit")]
        [TestMethod]
        public void LessThan_ConstantString()
        {
            var sut = new VtlEngine(new DataContainerFactory());
            ...
            using (var dataPointEnumerator = result.DataPoints.GetEnumerator())
            {
                ...
                Assert.AreEqual((StringType)"Gross Prod.", dataPointEnumerator.Current[0]);
                Assert.AreEqual((StringType)"2014", dataPointEnumerator.Current[1]);
                Assert.AreEqual((BooleanType)false, dataPointEnumerator.Current[2]);//ACC<ABC

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((StringType)"Population", dataPointEnumerator.Current[0]);
                Assert.AreEqual((StringType)"2013", dataPointEnumerator.Current[1]);
                Assert.AreEqual((BooleanType)true, dataPointEnumerator.Current[2]);//ABB<ABC

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((StringType)"Population", dataPointEnumerator.Current[0]);
                Assert.AreEqual((StringType)"2014", dataPointEnumerator.Current[1]);
                Assert.AreEqual((BooleanType)false, dataPointEnumerator.Current[2]);//ACB<ABC
            }
        }

        [TestCategory("Unit")]
        [TestMethod]
        public void LessThan_DataSetStringMeasure()
        {
            var sut = new VtlEngine(new DataContainerFactory());
            ...
            using (var dataPointEnumerator = result.DataPoints.GetEnumerator())
            {
                ...
                Assert.AreEqual((StringType)"Gross Prod.", dataPointEnumerator.Current[0]);
                Assert.AreEqual((StringType)"2013", dataPointEnumerator.Current[1]);
                Assert.AreEqual((BooleanType)true, dataPointEnumerator.Current[2]);//ABC<ACB

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((StringType)"Gross Prod.", dataPointEnumerator.Current[0]);
                Assert.AreEqual((StringType)"2014", dataPointEnumerator.Current[1]);
                Assert.AreEqual((BooleanType)false, dataPointEnumerator.Current[2]);//ACC<ABB

                dataPointEnumerator.MoveNext();
                Assert.AreEqual((StringType)"Population", dataPointEnumerator.Current[0]);
                Assert.AreEqual((StringType)"2013", dataPointEnumerator.Current[1]);
                Assert.AreEqual((BooleanType)true, dataPointEnumerator.Current[2]);//ABB<ACC
        ...
        [TestCategory("Unit")]
        [TestMethod]
        public void LessThan_timePeriod()
        {
