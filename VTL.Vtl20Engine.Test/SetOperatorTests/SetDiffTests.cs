...
namespace VTL.Vtl20Engine.Test.SetOperatorTests
{
    ...
    public class SetDiffTests
    {
        ...
        [TestCategory("Unit")]
        [TestMethod]
        public void SetDiff_TwoDataSets()
        {
            var sut = new VtlEngine(new DataContainerFactory());

            var DS_r = sut.Execute("DS_r <- setdiff(DS_1, DS_2);", new[] { _ds_1, _ds_2 })
        ...
        [TestCategory("Unit")]
        [TestMethod]
        public void SetDiff_EmptyResult()
        {
            var sut = new VtlEngine(new DataContainerFactory());

            var DS_r = sut.Execute("DS_r <- setdiff(DS_1, DS_1);", new[] { _ds_1, _ds_2 })
                .FirstOrDefault(r => r.Alias.Equals("DS_r"));
        ...
        [TestCategory("Unit")]
        [TestMethod]
        public void SetDiff_TwoOtherDataSets()
        {
            var sut = new VtlEngine(new DataContainerFactory());

            var DS_r = sut.Execute("DS_r <- setdiff(DS_1, DS_3);", new[] { _ds_1, _ds_3 })
                .FirstOrDefault(r => r.Alias.Equals("DS_r"));
        ...
        [TestCategory("Unit")]
        [TestMethod]
        public void SetDiff_NotMatchingStructure()
        {
            var sut = new VtlEngine(new DataContainerFactory());
            var e = Assert.ThrowsException<VtlException>(() => sut.Execute("DS_r <- setdiff(DS_1, DS_4);", new[] { _ds_1, _ds_4 })
                .FirstOrDefault(r => r.Alias.Equals("DS_r")));
            Assert.AreEqual("Alla dataset som ingår i SetDiff måste ha samma datsetkomponenter.", e.Message);
        }

      [TestCategory("Unit")]
        [TestMethod]
        public void SetDiff_ResultIsIntegerType()
        {
            var sut = new VtlEngine(new DataContainerFactory());
