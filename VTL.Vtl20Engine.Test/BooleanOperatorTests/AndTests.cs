...
namespace VTL.Vtl20Engine.Test.BooleanOperatorTests
{
    ...
    public class AndTests
    {
        ...
        [TestCategory("Unit")]
        [TestMethod]
        public void And_datasets()
        {
            var sut = new VtlEngine(new DataContainerFactory());

            var DS_r = sut.Execute("DS_r <- DS_1 and DS_2;", new[] { _ds_1, _ds_2 })
        ...
        [TestCategory("Unit")]
        [TestMethod]
        public void And_throwsErrorWhenMeasureIsString()
        {
            var sut = new VtlEngine(new DataContainerFactory());
            ...
            Assert.AreEqual("Värdekomponenten Id_4 har datatypen StringType vilken inte är tillåten för operatorn And.", ex.Message);
        }

        [TestCategory("Unit")]
        [TestMethod]
        public void And_datasetScalar()
        {
            var sut = new VtlEngine(new DataContainerFactory());

            var DS_r = sut.Execute("DS_r <- DS_1 and true;", new[] { _ds_1, _ds_2 })
                .FirstOrDefault(r => r.Alias.Equals("DS_r"));
        ...
        [TestCategory("Unit")]
        [TestMethod]
        public void And_datasetsWithConditions()
        {
            var sut = new VtlEngine(new DataContainerFactory());

            var DS_r = sut.Execute("DS_r <- DS_1#Id_1 = \"M\" and DS_1#Id_2 > 30;", new[] { _ds_1 })
                .FirstOrDefault(r => r.Alias.Equals("DS_r"));
